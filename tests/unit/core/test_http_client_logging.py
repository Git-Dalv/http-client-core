...
class TestHTTPClientLogging:
    ...
    def test_retry_attempts_logged(self):
        ...
        logging_config = LoggingConfig.create(level="INFO", format="json", enable_console=False)
        retry_config = RetryConfig(max_attempts=3, backoff_factor=1.0, backoff_base=0.1)
        config = HTTPClientConfig(
            base_url="https://api.example.com",
            logging=logging_config,
            retry=retry_config
        ...
        # Should have at least one "Retrying request" or "will retry" call
        retry_calls = [c for c in client._logger.warning.call_args_list if "retry" in c[0][0].lower()]
